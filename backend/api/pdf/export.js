...
export default async function handler(req,res){
 ...
 try{
  const id=String(req.query?.id||''),availability=String(req.query?.availability||''),admin=String(req.query?.admin||'');
  const query=id?'?id='+encodeURIComponent(id):admin==='1'?'?admin=1':availability==='1'?'?availability=1':'';
