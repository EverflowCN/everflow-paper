...
function vDuration(initial,onDone,dark=false){
 ...
 ruler.onscroll=()=>display(Math.max(0,Math.min(180,Math.round(ruler.scrollLeft/16))));ruler.querySelectorAll('[data-minute]').forEach(b=>b.onclick=()=>ruler.scrollTo({left:+b.dataset.minute*16,behavior:'smooth'}));
...
function vRenderData(){if(!V.detail||!['trend','pomodoro'].includes(page))return;vDetails.dataset.kind=page;
