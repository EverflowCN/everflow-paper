...
if(s.includes(v1Api))s=s.replace(v1Api,v2Api);else if(s.includes(originalApi))s=s.replace(originalApi,v2Api);
...
const hardenedLoad="async function loadAccount(){try{const x=await api('/api/auth/me'),u=x?.user||fallbackUser();if(!u?.id)return false;me=u;quota=x?.quota||null;return true}catch(e){if(e.status===401){forgetAuthToken();return false}const u=fallbackUser();if(codespacesAuthFallback&&u?.id){me=u;quota=null;return true}throw e}}";
if(s.includes(originalLoad))s=s.replace(originalLoad,hardenedLoad);
...
console.log('[Everflow] Codespaces auth fallback v2 applied.');
