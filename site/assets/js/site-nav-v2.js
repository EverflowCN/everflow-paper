(()=>{
  ...
  function build(container,mobile=false){
    ...
    for(const item of items){
      const a=document.createElement('a');a.href=item.href;a.textContent=item.label;if(item.match(path))a.classList.add('active');frag.appendChild(a);
    }
    ...
    const close=document.createElement('button');close.type='button';close.className='membership-nav-close ui-icon-only';close.innerHTML=closeIcon();close.setAttribute('aria-label','关闭购买会员入口');close.title='关闭';
  ...
  const topLayerOpen=()=>document.body.classList.contains('evera-modal-open')||document.body.classList.contains('evera-immersive-open')||document.body.classList.contains('relax-reset-open')||document.body.classList.contains('srs-immersive-fallback')||Boolean(document.fullscreenElement||document.webkitFullscreenElement);
