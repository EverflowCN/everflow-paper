(()=>{
  ...
  css.rel='stylesheet';css.href='/assets/css/zhenti-srs.css?v=20260823a';document.head.appendChild(css);
  ...
  home.innerHTML=`
    <section class="srs-hero">
      <div>
        ...
        <h1>408 速刷卡片</h1>
        <p>把已核验真题变成按遗忘速度自动到期的复习卡。答错、标记“不会/模糊”的题会优先进入队列；每次复习后按“重来 / 困难 / 记住 / 简单”重新计算下次复习时间。</p>
      </div>
      <div class="srs-stats">
    ...
    <section class="srs-toolbar">
      <div class="srs-filters"><span class="srs-toolbar-label">范围</span>${Object.entries(SUBJECTS).map(([k,v])=>`<button class="srs-filter${k==='all'?' active':''}" type="button" data-srs-filter="${k}">${v}</button>`).join('')}</div>
    ...
    <section class="srs-stage">
      ...
      <aside class="srs-side">
        <section class="srs-side-card"><h3>今日队列</h3><div class="srs-side-list"><div class="srs-side-row"><span>到期复习</span><b data-srs-side-due>0</b></div><div class="srs-side-row"><span>新卡额度</span><b data-srs-side-new>0</b></div><div class="srs-side-row"><span>今日已刷</span><b data-srs-reviewed>0</b></div><div class="srs-side-row"><span>已核验卡片</span><b data-srs-total>0</b></div></div></section>
  ...
  const els={due:$('[data-srs-due]'),newCount:$('[data-srs-new]'),retention:$('[data-srs-retention]'),streak:$('[data-srs-streak]'),filter:$$('[data-srs-filter]'),newLimit:$('[data-srs-new-limit]'),card:$('[data-srs-card]'),progress:$('[data-srs-progress]'),sideDue:$('[data-srs-side-due]'),sideNew:$('[data-srs-side-new]'),reviewed:$('[data-srs-reviewed]'),total:$('[data-srs-total]'),curve:$('[data-srs-curve]')};
  ...
  let filter='all',deck=[],loaded=false,loading=false,selected=null,revealed=false,currentId=null,startedAt=0,skipped=new Set();
  ...
  function queueInfo(){
    ...
    const eligible=deck.filter(subjectMatch).filter(c=>!skipped.has(c.id));
  ...
  function averageRetention(){const vals=deck.filter(subjectMatch).map(c=>recallOf(state.cards[c.id])).filter(v=>v!=null);if(!vals.length)return null;return vals.reduce((a,b)=>a+b,0)/vals.length}

  async function loadDeck(){
    if(loaded||loading)return;loading=true;
    try{
      const manifest=await fetch('/data/zhenti/manifest.json?v=20260823srs',{cache:'no-store'}).then(r=>r.ok?r.json():null);
      ...
      const groups=await Promise.all(entries.map(async([year,meta])=>{
        const paper=await fetch(`/data/zhenti/${year}.json?v=20260823srs`,{cache:'no-store'}).then(r=>r.ok?r.json():null).catch(()=>null);
        if(!paper)return[];
  ...
  function renderStats(){
    ...
    els.sideDue.textContent=String(info.due.length);els.sideNew.textContent=`${Math.min(info.fresh.length,info.newSlots)}/${info.newSlots}`;els.reviewed.textContent=String(day.reviews||0);els.total.textContent=String(deck.filter(subjectMatch).length);
  ...
  function renderCurrent(){
    ...
    if(!card){
      els.card.innerHTML=`<div class="srs-empty"><div><div class="mark">✓</div><h2>今日速刷完成</h2><p>当前范围内没有到期卡片，或今日新卡额度已经用完。到期时间会按每张卡的稳定度自动安排。</p><button type="button" data-srs-reset-skip>${skipped.size?'恢复本次跳过':'重新检查队列'}</button></div></div>`;
    ...
    els.card.innerHTML=`<article class="srs-card"><div class="srs-card-meta"><span class="srs-chip ${isNew?'new':'due'}">${isNew?'新卡':'到期复习'}</span><span class="srs-chip">${card.year} · 第 ${card.q} 题</span><span class="srs-chip">${SHORT[card.subject]||''} ${SUBJECTS[card.subject]||''}</span><div class="srs-memory"><span>${memory==null?'尚未建立曲线':`预计记忆率 ${memory}%`}</span><span class="srs-memory-bar"><i style="width:${memory==null?100:memory}%"></i></span></div></div><div class="srs-question"><strong>${item.type==='comprehensive'?'综合应用题':'选择题'}</strong><div>${esc(item.stem)}</div></div>${options}${primary}${answer}${grades}<div class="srs-card-foot"><span>队列剩余 ${info.queue.length} 张 · 已核验来源优先 · 弱项自动前置</span><button class="srs-skip" type="button" data-srs-skip>跳过这张 <kbd>S</kbd></button></div></article>`;
  ...
  els.filter.forEach(btn=>btn.addEventListener('click',()=>{filter=btn.dataset.srsFilter||'all';els.filter.forEach(x=>x.classList.toggle('active',x===btn));skipped.clear();currentId=null;selected=null;revealed=false;renderAll()}));
  els.newLimit.value=String(state.settings.dailyNew||20);els.newLimit.addEventListener('change',()=>{state.settings.dailyNew=Number(els.newLimit.value)||20;save();skipped.clear();currentId=null;renderAll()});
