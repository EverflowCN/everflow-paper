(()=>{
  ...
  function patchRecord(year,q,patch){
    ...
    saveRecords();document.dispatchEvent(new CustomEvent('everflow:zhenti-records-change',{detail:{year,q,source:'graph'}}));return records[key]||{};
  }
  ...
  async function primeSubjects(){
    ...
    render();if(selected)updateCell(`${selected.year}-${selected.q}`);
  }
  ...
  drawerStatuses.forEach(btn=>btn.addEventListener('click',()=>{if(!selected)return;const status=btn.dataset.drawerStatus||undefined;patchRecord(selected.year,selected.q,{status});updateCell(`${selected.year}-${selected.q}`);syncDrawerControls()}));
  document.addEventListener('keydown',event=>{if(event.key==='Escape'&&!drawer.hidden){hideDrawer();event.preventDefault()}});

  function refresh(){records=loadRecords();try{current=localStorage.getItem(CURRENT_KEY)||current}catch{}if(!parseKey(current))current=latestKey(records);render();if(selected)syncDrawerControls()}
  window.addEventListener('pageshow',event=>{if(event.persisted)refresh()});window.addEventListener('storage',event=>{if(event.key===STORAGE_KEY||event.key===CURRENT_KEY)refresh()});document.addEventListener('everflow:zhenti-records-change',refresh);

  render({focusCurrent:true});const initial=parseKey(current)||{year:2026,q:1};openQuestion(initial.year,initial.q);primeSubjects().catch(err=>console.error('Everflow graph subject index failed',err));
})();
