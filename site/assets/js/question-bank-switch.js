...
if(view==='zhenti'){
  ...
  css.rel='stylesheet';
  css.href='/assets/css/question-bank-switch.css?v=20260824-bank8';
  document.head.appendChild(css);

  const shell=document.createElement('section');
  shell.className='bank-source-shell';
  shell.setAttribute('aria-label','题库切换');
  shell.innerHTML=`<div class="bank-source-inner"><span class="bank-source-label">题库</span><div class="bank-source-segmented" role="tablist"><button type="button" data-bank-source="zhenti" class="${current==='zhenti'?'active':''}">408 真题</button><button type="button" data-bank-source="relax1000" class="${current==='relax1000'?'active':''}">Relax1000</button><button type="button" data-bank-practice>组卷</button></div><span class="bank-source-note">${current==='relax1000'?'1576 道选择题 · 图谱 / 速刷卡片':'2009—2026 · 已核验真题'}</span></div>`;
  ...
  if(current==='relax1000'){
    import('/assets/js/relax1000-graph.js?v=20260824-bank8')
      .then(()=>import('/assets/js/relax1000-cards.js?v=20260824-cards1'))
      .catch(error=>{
