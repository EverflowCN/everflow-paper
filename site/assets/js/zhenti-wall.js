(()=>{
  ...
  const SOURCE_LABEL={tommy408:'TommyTay0712/408',neville408:'408-exam-paper',csgraduates:'计算机考研杂货铺',foreverlink:'408 ForeverLink',hermes408:'Hermes 408',noobdream:'N诺'};
  const SUBJECTS={
  ...
  async function loadPaper(year){
    ...
    const promise=fetch(`${DATA_BASE}/${year}.json?v=20260823e`,{cache:'no-store'})
  ...
  function isDone(r){return hasSubmitted(r)||Boolean(r.reviewed)||Boolean(r.status)}
  ...
  let subject=localStorage.getItem('everflow-408-wall-subject')||'ds';
  ...
  function toggleTimer(){
    if(timerStartedAt){
      timerAccumulated+=Date.now()-timerStartedAt;timerStartedAt=0;clearInterval(timerInterval);timerInterval=null;els.timerText.textContent=formatShort(timerAccumulated);return;
    }
    timerStartedAt=Date.now();els.timerText.textContent=formatShort(timerAccumulated);
    timerInterval=setInterval(()=>{els.timerText.textContent=formatShort(modalElapsed())},1000);
  }
  function startWholeTimer(){
    clearInterval(fullTimerInterval);fullTimerAccumulated=0;fullStartedAt=Date.now();els.paperTimer.textContent='00:00:00';
    fullTimerInterval=setInterval(()=>{els.paperTimer.textContent=formatLong(fullElapsed())},1000);
  }
  function toggleWholeTimer(){
    if(fullStartedAt){
      fullTimerAccumulated+=Date.now()-fullStartedAt;fullStartedAt=0;clearInterval(fullTimerInterval);fullTimerInterval=null;els.paperTimer.textContent=formatLong(fullTimerAccumulated);return;
    }
  ...
  function renderQuestionHtml(item,q,r,{context='modal'}={}){
    ...
    if(choice){
      if(submitted){
        result=`<div class="answer-result ${r.correct?'correct':'wrong'}"><strong>${r.correct?'✓ 回答正确':'✕ 回答错误'}</strong><span>你的答案：${esc(r.answer)} · 正确答案：${esc(item.answer)}</span></div>`;
      }
      const primaryLabel=submitted?'下一题 →':'提交答案';
      const disabled=!submitted&&!r.draftAnswer;
      ...
      result=`<div class="answer-actions comprehensive-actions"><button type="button" class="answer-submit" data-answer-reveal>${showAnswer?'收起参考答案':'查看参考答案'}<kbd>Enter</kbd></button></div>`;
    }
    ...
    const shortcut=choice
      ?`<div class="shortcut-inline">A/B/C/D 选项 · Enter 提交/下一题 · ←/→ 或 J/K 切题 · 1/2/3 熟练度 · R 解析 · N 笔记 · T 计时 · F 收藏</div>`
      :`<div class="shortcut-inline">Enter 查看答案 · ←/→ 或 J/K 切题 · 1/2/3 自评 · N 笔记 · F 收藏</div>`;
  ...
  async function renderInto(box,year,q,context){
    const token=`${year}-${q}-${Date.now()}`;
    box.dataset.loadToken=token;
    box.innerHTML='<div class="question-loading">正在读取并核验题库…</div>';
    const item=await getQuestionData(year,q);
    if(box.dataset.loadToken!==token)return;
    box.innerHTML=renderQuestionHtml(item,q,record(year,q),{context});
    attachQuestionInteractions(box,year,q,context,item);
    if(context==='modal'){
      if(item?.verification?.status==='verified'){
        els.analysis.innerHTML=`<strong>参考答案：${esc(item.answer)}</strong><p>${esc(item.analysis||'暂无解析')}</p><div class="question-source-note">来源核验：${esc(sourceText(item))}</div>`;
      }else{
        els.analysis.innerHTML='<strong>解析</strong><p>该题尚未核验，因此不展示答案和解析。</p>';
      }
    }
  }

  function refreshAfterRecordChange(context){
    renderSummary();renderMatrix();renderList();if(sideMode==='year')renderSide();
    if(mode==='whole')renderWholeHome();
    if(context==='paper')renderPaperSession(false);else renderModal(false);
  }

  async function submitChoice(year,q,context,item){
    const r=record(year,q),answer=r.draftAnswer;
    if(!answer||!item)return;
    const correct=String(answer)===String(item.answer);
    const spent=Math.max(0,Date.now()-(context==='paper'?paperQuestionStartedAt:modalQuestionStartedAt));
    patchRecord(year,q,{
      answer,
      draftAnswer:undefined,
      correct,
      attempts:Number(r.attempts||0)+1,
      correctCount:Number(r.correctCount||0)+(correct?1:0),
      timeSpent:Number(r.timeSpent||0)+Math.round(spent/1000),
      lastAnsweredAt:new Date().toISOString()
    });
    if(context==='modal')els.analysis.hidden=false;
    refreshAfterRecordChange(context);
  }

  function toggleReveal(year,q,context){
    const r=record(year,q);
    patchRecord(year,q,{reviewed:!r.reviewed});
    refreshAfterRecordChange(context);
  }

  function stepByContext(context,delta){
    if(context==='paper')stepWhole(delta);else stepQuestion(delta);
  }

  function attachQuestionInteractions(box,year,q,context,item){
    box.querySelectorAll('[data-answer-option]').forEach(btn=>btn.addEventListener('click',()=>{
      const r=record(year,q);if(hasSubmitted(r))return;
      patchRecord(year,q,{draftAnswer:btn.dataset.answerOption});
      if(context==='paper')renderPaperSession(false);else renderModal(false);
    }));
    const submit=box.querySelector('[data-answer-submit]');
    if(submit)submit.addEventListener('click',()=>{
      const r=record(year,q);
      if(hasSubmitted(r)){stepByContext(context,1);return}
      submitChoice(year,q,context,item);
    });
    const reveal=box.querySelector('[data-answer-reveal]');
    if(reveal)reveal.addEventListener('click',()=>toggleReveal(year,q,context));
    const favorite=box.querySelector('[data-answer-favorite]');
    if(favorite)favorite.addEventListener('click',()=>{
      patchRecord(year,q,{favorite:!record(year,q).favorite});
      if(context==='paper')renderPaperSession(false);else renderModal(false);
    });
  }
  ...
  function subjectStats(years=visibleYears()){
    ...
    years.forEach(y=>qs.forEach(q=>{const r=record(y,q);if(isDone(r))done++;if(r.status==='fuzzy'||r.status==='weak')weak++}));
  ...
  function renderMatrix(){
    const qs=SUBJECTS[subject].questions,years=visibleYears();els.matrixHead.innerHTML=`<tr><th class="freq-head">题</th><th class="year-head">年份</th>${qs.map(q=>`<th class="q-head">${q}</th>`).join('')}</tr>`;els.matrixBody.innerHTML='';
    years.forEach(year=>{
      const tr=document.createElement('tr'),qCells=qs.map(q=>{const r=record(year,q),state=answerState(r);return`<td class="question-cell"><button class="matrix-q ${state}${masteryClass(r)}" type="button" data-year="${year}" data-q="${q}" aria-label="${year} 年第 ${q} 题">${q}</button></td>`}).join('');
  ...
  function renderList(){
    els.yearList.innerHTML='';visibleYears().forEach(year=>{const s=yearStats(year),card=document.createElement('article');card.className='year-list-card';card.innerHTML=`<div class="year-list-head"><strong>${year}</strong><span>${s.done}/${s.total} · ${s.rate}%</span></div><div class="year-list-progress"><i style="width:${s.rate}%"></i></div><div class="year-list-meta"><span>答对 ${s.correct}</span><span>答错 ${s.wrong}</span><span>模糊 ${s.fuzzy}</span><span>不会 ${s.weak}</span></div><button type="button">进入 ${year} 真题</button>`;card.querySelector('button').addEventListener('click',()=>openQuestion(year,SUBJECTS[subject].questions[0]));els.yearList.appendChild(card)});
  }
  ...
  function renderWholeHome(){
    ...
    YEARS.slice().reverse().forEach(year=>{
      const s=wholeYearStats(year),card=document.createElement('article');card.className='whole-year-card';
      const mini=ALL_QUESTIONS.map(q=>{const r=record(year,q);return`<i class="${answerState(r)}${masteryClass(r)}"></i>`}).join('');
      card.innerHTML=`<div class="whole-year-head"><div><span>408 · FULL PAPER</span><strong>${year}</strong></div><b>${s.rate}%</b></div><div class="whole-year-progress"><i style="width:${s.rate}%"></i></div><div class="whole-year-meta"><span>已做 <b>${s.done}/47</b></span><span>答对 <b>${s.correct}</b></span><span>答错 <b>${s.wrong}</b></span><span>不会 <b>${s.weak}</b></span></div><div class="whole-mini-grid">${mini}</div><button class="whole-start" type="button">${s.done?'继续整套':'开始整套'} →</button>`;
      card.querySelector('.whole-start').addEventListener('click',()=>openWholePaper(year));els.wholeYearGrid.appendChild(card);
    });
  }

  function renderModal(loadQuestion=true){
    if(selectedYear==null||selectedQuestion==null)return;
    const r=record(selectedYear,selectedQuestion),subKey=subjectForQuestion(selectedQuestion),sub=SUBJECTS[subKey];
    ...
    els.modalStatuses.forEach(btn=>btn.classList.toggle('active',Boolean(btn.dataset.modalStatus)&&btn.dataset.modalStatus===r.status));
    els.note.value=r.note||'';els.noteState.textContent=r.updatedAt?`已保存 · ${new Date(r.updatedAt).toLocaleString('zh-CN',{hour12:false})}`:'本机自动保存';
    ...
    if(loadQuestion)renderInto(els.modalQuestionBox,selectedYear,selectedQuestion,'modal');else renderInto(els.modalQuestionBox,selectedYear,selectedQuestion,'modal');
  }
  ...
  function stepQuestion(delta){
    const qs=SUBJECTS[subject].questions,i=qs.indexOf(selectedQuestion),next=qs[i+delta];
  ...
  function openWholePaper(year){
    fullYear=year;const firstUnanswered=ALL_QUESTIONS.find(q=>!isDone(record(year,q)));fullQuestion=firstUnanswered||1;paperQuestionStartedAt=Date.now();
    els.paperSession.hidden=false;document.body.style.overflow='hidden';startWholeTimer();renderPaperSession();
  }
  ...
  function renderPaperSession(){
    ...
    const s=wholeYearStats(fullYear),r=record(fullYear,fullQuestion),subKey=subjectForQuestion(fullQuestion),sub=SUBJECTS[subKey];
    els.paperYear.textContent=String(fullYear);els.paperProgress.textContent=`${s.done}/47`;els.paperPercent.textContent=`${s.rate}%`;els.paperProgressBar.style.width=`${s.rate}%`;els.paperSubject.textContent=`${sub.short} ${sub.name}`;els.paperType.textContent=fullQuestion<=40?'选择题':'综合应用题';els.paperCurrent.textContent=String(fullQuestion);
    els.paperNote.value=r.note||'';els.paperNoteState.textContent=r.updatedAt?`已保存 · ${new Date(r.updatedAt).toLocaleString('zh-CN',{hour12:false})}`:'本机自动保存';
    els.paperStatuses.forEach(btn=>btn.classList.toggle('active',Boolean(btn.dataset.paperStatus)&&btn.dataset.paperStatus===r.status));
    els.paperPrev.disabled=fullQuestion<=1;els.paperNext.disabled=fullQuestion>=47;
    els.paperAnswerGrid.innerHTML=ALL_QUESTIONS.map(q=>{const rr=record(fullYear,q);return`<button type="button" class="paper-answer ${answerState(rr)}${masteryClass(rr)}${q===fullQuestion?' current':''}" data-paper-jump="${q}" title="第 ${q} 题">${q}</button>`}).join('');
    els.paperAnswerGrid.querySelectorAll('[data-paper-jump]').forEach(btn=>btn.addEventListener('click',()=>{fullQuestion=Number(btn.dataset.paperJump);paperQuestionStartedAt=Date.now();renderPaperSession()}));
    renderInto(els.paperQuestionBox,fullYear,fullQuestion,'paper');
  }
  ...
  function setMastery(context,status){
    const year=context==='paper'?fullYear:selectedYear,q=context==='paper'?fullQuestion:selectedQuestion;if(year==null||q==null)return;
    patchRecord(year,q,{status});refreshAfterRecordChange(context);
  }

  function activeContext(){
    if(els.paperSession&&!els.paperSession.hidden)return'paper';
    if(els.modal&&!els.modal.hidden)return'modal';
    return null;
  }
  function isTyping(){
  ...
  function shortcutOption(context,key){
    const btn=currentBox(context)?.querySelector(`[data-answer-option="${key}"]`);if(btn&&!btn.disabled)btn.click();
  }
  function shortcutEnter(context){
    const box=currentBox(context),submit=box?.querySelector('[data-answer-submit]'),reveal=box?.querySelector('[data-answer-reveal]');
    if(submit){submit.click();return}
    if(reveal){reveal.click();return}
    stepByContext(context,1);
  }
  function shortcutReveal(context){
    if(context==='modal'){els.analysis.hidden=!els.analysis.hidden;return}
    const box=currentBox(context),reveal=box?.querySelector('[data-answer-reveal]');if(reveal)reveal.click();
  }
  function shortcutNote(context){
    if(context==='modal'){els.noteBox.hidden=false;els.note.focus()}else{els.paperNote.focus()}
  }
  function shortcutFavorite(context){
    const btn=currentBox(context)?.querySelector('[data-answer-favorite]');if(btn)btn.click();
  }
  ...
  function installShortcutHelp(){
    ...
    const open=()=>{overlay.hidden=false};const close=()=>{overlay.hidden=true};
    fab.addEventListener('click',open);overlay.querySelectorAll('[data-shortcut-close]').forEach(el=>el.addEventListener('click',close));
    overlay._open=open;overlay._close=close;window.__everflowShortcutHelp=overlay;
    if(!localStorage.getItem(SHORTCUT_TIP_KEY)&&matchMedia('(min-width:900px)').matches){
      const tip=document.createElement('div');tip.className='shortcut-tip';tip.innerHTML='电脑刷题：<b>A–D</b> 选项，<b>Enter</b> 提交，<b>← →</b> 切题，按 <b>?</b> 查看全部快捷键。';document.body.appendChild(tip);
      localStorage.setItem(SHORTCUT_TIP_KEY,'1');setTimeout(()=>tip.classList.add('show'),100);setTimeout(()=>{tip.classList.remove('show');setTimeout(()=>tip.remove(),250)},6500);
    }
  }
  ...
  els.paperStatuses.forEach(btn=>btn.addEventListener('click',()=>setMastery('paper',btn.dataset.paperStatus||'')));
  ...
  if(els.paperTimer?.parentElement){els.paperTimer.parentElement.title='点击开始 / 暂停计时（T）';els.paperTimer.parentElement.style.cursor='pointer';els.paperTimer.parentElement.addEventListener('click',toggleWholeTimer)}
  ...
  document.addEventListener('keydown',e=>{
    ...
    const help=window.__everflowShortcutHelp;
    if(key==='Escape'&&help&&!help.hidden){e.preventDefault();help._close();return}
    const context=activeContext();if(!context)return;
    if(key==='Escape'){e.preventDefault();if(context==='paper')closeWholePaper();else closeQuestion();return}
    const upper=key.toUpperCase();
    if(['A','B','C','D'].includes(upper)){e.preventDefault();shortcutOption(context,upper);return}
    if(key==='Enter'){e.preventDefault();shortcutEnter(context);return}
    if(key==='ArrowLeft'||upper==='J'){e.preventDefault();stepByContext(context,-1);return}
    if(key==='ArrowRight'||upper==='K'){e.preventDefault();stepByContext(context,1);return}
    if(key==='1'||key==='2'||key==='3'){e.preventDefault();setMastery(context,key==='1'?'mastered':key==='2'?'fuzzy':'weak');return}
    if(upper==='R'){e.preventDefault();shortcutReveal(context);return}
    if(upper==='N'){e.preventDefault();shortcutNote(context);return}
    if(upper==='T'){e.preventDefault();context==='paper'?toggleWholeTimer():toggleTimer();return}
  ...
  setupRangeSelects();installShortcutHelp();renderMode();
})();