(()=>{
  ...
  async function loadPaper(year){
    ...
    const promise=fetch(`${DATA_BASE}/${year}.json?v=20260823d`,{cache:'no-store'})
  ...
  function renderQuestionHtml(item,q,{showAnswer=false}={}){
    if(!item||item.verification?.status!=='verified'){
      return `<div class="question-live question-unverified"><div class="question-verify-row"><span class="verify-badge pending">待核验</span></div><div class="question-stem"><span class="question-number">（${q}）</span><p>该题尚未完成年份与题号的来源核验，暂不展示题干。核验通过后才会进入正式题库。</p></div></div>`;
    }
    const options=item.options?`<div class="live-options">${Object.entries(item.options).map(([k,v])=>`<div class="live-option"><b>${esc(k)}.</b><span>${esc(v)}</span></div>`).join('')}</div>`:'';
    const answer=showAnswer?`<div class="live-answer"><strong>参考答案：${esc(item.answer)}</strong><p>${esc(item.analysis||'')}</p></div>`:'';
    return `<div class="question-live"><div class="question-verify-row"><span class="verify-badge">已核验</span><span class="verify-note">${esc(item.verification?.mode==='cross-checked-paraphrase'?'题干转述版':'已核验题干')}</span></div><div class="question-stem"><span class="question-number">（${q}）</span><p>${esc(item.stem)}</p></div>${options}${answer}<div class="question-source-note">交叉核验：${esc(sourceText(item))}</div></div>`;
  }
  async function hydrateModalQuestion(year,q){
    const token=`${year}-${q}`;
    els.modalQuestionBox.dataset.loadToken=token;
    els.modalQuestionBox.innerHTML=`<div class="question-loading">正在读取并核验题库…</div>`;
    els.analysis.innerHTML='<strong>解析</strong><p>正在读取已核验解析…</p>';
    const item=await getQuestionData(year,q);
    if(els.modalQuestionBox.dataset.loadToken!==token)return;
    els.modalQuestionBox.innerHTML=renderQuestionHtml(item,q);
  ...
  async function hydrateWholeQuestion(year,q){
    const token=`${year}-${q}`;
    els.paperQuestionBox.dataset.loadToken=token;
    els.paperQuestionBox.innerHTML='<div class="question-loading">正在读取并核验题库…</div>';
    const item=await getQuestionData(year,q);
    if(els.paperQuestionBox.dataset.loadToken!==token)return;
    els.paperQuestionBox.innerHTML=renderQuestionHtml(item,q,{showAnswer:false});
  }
  ...
  function renderMatrix(){
    ...
    els.matrixHead.innerHTML=`<tr><th class="freq-head">题</th><th class="year-head">年份</th>${qs.map(q=>`<th class="q-head">${q}</th>`).join('')}</tr>`;els.matrixBody.innerHTML='';
    years.forEach(year=>{const tr=document.createElement('tr'),qCells=qs.map(q=>{const status=record(year,q).status||'unmarked';return`<td class="question-cell"><button class="matrix-q ${status}" type="button" data-year="${year}" data-q="${q}" aria-label="${year} 年第 ${q} 题，${status==='unmarked'?'未做':STATUS_LABEL[status]}">${q}</button></td>`}).join('');tr.innerHTML=`<td class="freq-cell"><span class="freq-badge">${qs.length}</span></td><td class="year-cell">${year}</td>${qCells}`;els.matrixBody.appendChild(tr)});
  ...
  function renderModal(){
    ...
    hydrateModalQuestion(selectedYear,selectedQuestion);
  }
