...
const css=document.createElement('link');css.rel='stylesheet';css.href='/assets/css/question-bank-switch.css?v=20260824-graph3';document.head.appendChild(css);
...
shell.innerHTML=`<div class="bank-source-inner"><span class="bank-source-label">图谱</span><div class="bank-source-segmented" role="tablist"><button type="button" data-graph-source="zhenti" class="${current==='zhenti'?'active':''}">408 真题</button><button type="button" data-graph-source="relax1000" class="${current==='relax1000'?'active':''}">Relax1000</button></div><span class="bank-source-note">${current==='relax1000'?'1576 道题 · 每行最多 45 格 · 放大 / 适应宽度':'2009—2026 · 18 年 × 47 题 · 放大 / 适应屏幕'}</span></div>`;
...
if(current==='relax1000'){
  import('/assets/js/relax1000-graph.js?v=20260824-graph45-2').catch(error=>{
    ...
    fail.innerHTML='<section><h1>Relax1000 图谱载入失败</h1><p>没有成功读取图谱数据，请刷新后重试。</p></section>';
    shell.after(fail);
  });
}
