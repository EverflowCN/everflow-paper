...
## 2026-09-17 课程选择改为纵向展开列表

- `/408/` 课程导航改为当前课程摘要与纵向展开列表，名称换行、显示课时数与选中状态，避免横向滑动。
- 选择后自动收起并恢复焦点；支持键盘与 Escape 收起，链接同步当前课程。
- 保留课程稳定 ID、打卡与备注存储逻辑；独立样式适配窄屏与深色主题。
- 本地 JavaScript 语法与 diff 检查通过；浏览器无法访问本地预览，线上交互待部署验证。


## 2026-09-17 课程卡片与视频时长汇总

- 课程选择改为紧凑两列卡片，手机单列；选中态使用红色描边，课程名自动换行。关键样式随 HTML 内联，避免独立 CSS 缓存失效时出现裸按钮。
- 每门课程自动汇总视频时长；当前科目展示总时长、按打卡计算的完成时长和剩余时长。缺失时长单独提示，不冒充真实观看计时。
- 支持 MM:SS 与 HH:MM:SS；保留课程 ID、学习状态和备注。缓存版本升至 v70。
