...
export function validateState(s){
 ...
 const p=s.settings;if(p){if(p.focusBackground!==undefined&&p.focusBackground!==null&&!/^#[a-fA-F0-9]{6}$/.test(p.focusBackground))throw Error('背景颜色格式不正确');for(const key of ['notifications','vibration','hour24'])if(p[key]!==undefined&&typeof p[key]!=='boolean')throw Error('提醒设置格式不正确');if(p.weekStart!==undefined&&![0,1].includes(p.weekStart))throw Error('每周起始日不正确');if(p.tagSort!==undefined&&!['manual','duration','name'].includes(p.tagSort))throw Error('标签排序格式不正确');if(p.wish!==undefined&&(typeof p.wish!=='string'||p.wish.length>2000))throw Error('许愿内容过长')}if(p&&p.defaultFruit==null)p.defaultFruit='tomato';
