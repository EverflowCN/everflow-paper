...
const safeOptions = (bank: string, value: unknown) => {
  if (bank === 'relax1000') {
    ...
    return rows.length ? rows.slice(0, 4) : undefined
  }
...
Deno.serve(async req => {
  ...
  try {
    ...
    if (action === 'save') {
      ...
      const { data, error } = await admin.from('question_overrides').upsert(row, { onConflict: 'bank,entity_id' }).select('id,bank,entity_id,patch,enabled,revision,created_at,updated_at').single()
      if (error) throw error
    ...
    if (action === 'restore') {
      ...
      const revision = Number(existing.revision || 0) + 1
      const { data, error } = await admin.from('question_overrides').update({ enabled: false, revision, updated_at: new Date().toISOString() }).eq('id', existing.id).select('id,bank,entity_id,patch,enabled,revision,created_at,updated_at').single()
      if (error) throw error
